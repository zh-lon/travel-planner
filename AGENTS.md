...
**所有 AI 调用必须使用流式传输**，禁止新增非流式 `chat()` 调用。

| 函数 | 用途 | 约束 |
|---|---|---|
| `chatStream()` | 纯文本流式输出 | 主力函数，所有文本生成场景使用 |
| `chatWithToolsStream()` | 流式工具调用（Agent 循环） | agent 循环专用，避免中间代理 504 |
| `chat()` | 非流式 | **仅限现有 3 处秒级场景**：意图识别（`maxTokens=8`）、连通性测试（`maxTokens=256`）、坐标验证（`geo-verify.ts`，静默失败兜底）。**禁止新增调用** |

**原因**：非流式调用在 AI 生成完整响应期间连接无数据流动，中间代理（Nginx/云网关）默认 60s 读超时即返回 HTTP 504，即使 AI 服务已成功。流式调用每收到 token 即发送数据，代理超时持续重置。

超时采用**空闲超时**语义：每次收到上游数据块即重置计时器，默认 10 分钟无数据才中断（`timeoutMs=0` 可禁用）。轻量流式接口（意图解析、地点攻略）保持 3 分钟空闲超时。Agent 循环的 `chatWithToolsStream` 默认 5 分钟空闲超时。

客户端通过 `postSse()` 接收事件流，SSE 事件类型：`step`、`status`、`delta`、`thinking`、`tool_call`、`tool_result`、`result`、`error`。
