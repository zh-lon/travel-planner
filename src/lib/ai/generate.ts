...
// 生成行程时使用：不含住宿和餐饮类型（用户自行安排住宿和餐饮）
export const ITEM_JSON_SHAPE_NO_STAY = `{"type":"SIGHT|TRANSPORT|SHOPPING|OTHER","title":"标题","startTime":"09:00","endTime":"11:00","placeName":"可在高德地图搜到的准确地点名，无固定地点则填空字符串","city":"该地点所在城市名，必须与目的地中列出的城市完全一致；单城市目的地填该城市；交通/无固定地点填空字符串","estimatedCost":100,"needBooking":false,"notes":"一句推荐理由或实用提示"}`;

// 生成行程时使用：不含住宿和餐饮类型
const JSON_CONTRACT_NO_STAY = `你只输出一个 JSON 对象，不要任何解释文字，不要 Markdown 代码块。JSON 结构如下：
{"days":[{"theme":"当天主题","items":[${ITEM_JSON_SHAPE_NO_STAY}]}]}
硬性要求：
1. days 数组长度必须严格等于行程天数，顺序即第 1..N 天；
2. 每天安排 4~7 个行程项：含景点游览；首日含抵达交通，末日含返程；
...
2. 同一天内时间从早到晚、互不重叠；相邻行程项地理位置尽量就近，动线合理；
3. 如果目的地含多个城市，城市间转移当天安排城际交通行程项（type 为 TRANSPORT，title 如「昆明→大理 高铁」，placeName 填到达站点或留空）；theme 体现当天所在城市；
4. placeName 用规范 POI 名称，交通/自由活动等留空字符串；
5. city 字段填写该行程项所在城市，必须与目的地中列出的某个城市名完全一致；交通行程项的 city 填出发城市；无固定地点的项填空字符串；
6. estimatedCost 为人均预估费用（元，可为 0）；全部使用中文；
7. needBooking 表示该地点是否需要提前预约。`;
...
export function buildCreatePrompt(p: GenerateParams): ChatMessage[] {
  ...
  return [
    { role: "system", content: `你是资深的国内旅行规划师。请为用户规划一份实用、合理的行程方案。${JSON_CONTRACT_NO_STAY}
额外质量要求：
1. 景点安排在常见开放时间内（博物馆/展馆注意周一闭馆）；
2. 各项费用预估符合用户选择的预算档位。` },
...
export function buildGuidePrompt(p: GuideParams): ChatMessage[] {
  ...
  return [
    {
      role: "system",
      content: `你是资深的旅行行程整理师。用户会提供一段旅行攻略文本（可能来自小红书等社交平台，含表情符号、话题标签、口语化表达）。你的任务是把攻略内容提取并整理成按天的结构化行程。${JSON_CONTRACT_NO_STAY}
针对攻略提取的额外要求：
...
2. 时段合理性：景点是否排在常见开放时间内（博物馆/展馆类注意周一闭馆）、夜间活动是否错排在白天、相邻项目时间有无重叠；
3. 节奏合理性：单日是否过满（赶场）或过空；
4. 跨城衔接：多城市行程中城际交通项是否合理衔接前后天的城市。
对不合格项直接微调（调整顺序、修正时间或替换为合理方案）：保持 JSON 结构与字段格式完全一致，days 数组长度不变，尽量保留原地点名称与主题。不要新增住宿（HOTEL）或餐饮（FOOD）类行程项，保持原有行程项类型不变。若全部合格，原样返回该 JSON。你只输出一个纯 JSON 对象，不要任何解释文字，不要 Markdown 代码块。`,
      },
      {
        role: "user",
